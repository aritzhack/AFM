...
public class ContainerFabricator extends AFMContainer {
    ...
    public void update() {
        ...
        for (int x = 0; x < this.craftMatrix.getSizeInventory(); x++) {
            ...
            if (!this.checkInventoryForNeeded(needed) &&
                    !this.checkNeighboursForNeeded(needed))
    ...
    private boolean checkInventoryForNeeded(ItemStack neededStack) {
        Iterator<ItemStack> iterator = this.tempInv.iterator();
        return checkForISinIterator(iterator, neededStack);
    }

    private boolean checkNeighboursForNeeded(ItemStack neededStack) {
     ...
     */
    private boolean checkForISinIterator(Iterator<ItemStack> iterator, ItemStack needed) {
        if (iterator == null) return false;
        while (iterator.hasNext()) {
            ItemStack curr = iterator.next();
