...
public class ContainerChestTest extends Container {
    ...
    private final TETestChest tileEntity;

    public ContainerChestTest(final TETestChest tileEntity, final InventoryPlayer player_inventory) {
        this.tileEntity = tileEntity;
        // Add chest's slots
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 9; x++) {

                final Slot s = new Slot(tileEntity, x + y * 9, 8 + x * 18, 18 + y * 18);
                s.setBackgroundIcon(Item.diamond.getIconFromDamage(0)); // Diamond with
                                                                        // meta=0
