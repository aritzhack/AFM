...
public final class UtilAFM {
    ...
    private static int sortItemStack(final ItemStack o1, final ItemStack o2) {
    ...
    public static void initOreDict() {
        for (final String s : OreDictionary.getOreNames()) {
            for (final ItemStack is : OreDictionary.getOres(s)) {
                UtilAFM.oreDictMap.put(is, s);
            }
        }
    }

    public static void dropEntityItem(final ItemStack stack, final World world, final int x, final int y, final int z) {
        if (world.isRemote) return;
        final double xD = world.rand.nextFloat() * 0.7D + (1.0D - 0.7D) * 0.5D;
        final double yD = world.rand.nextFloat() * 0.7D + (1.0D - 0.7D) * 0.5D;
        final double zD = world.rand.nextFloat() * 0.7D + (1.0D - 0.7D) * 0.5D;
        final EntityItem item = new EntityItem(world, x + xD, y + yD, z + zD, stack);
     ...
     *         -1 If exactly equal (both null means equal if notNull) (NBT equal or both
     *         null)<br />
     * <br />
     *         <p/>
     ...
     *         4 If one is null <br />
     */
    private static int compareItemStacks(final ItemStack s1, final ItemStack s2, final boolean amountStrict, final boolean notNull) {
    ...
    public static boolean isSameItem(final ItemStack s1, final ItemStack s2) {
        return UtilAFM.compareItemStacks(s1, s2, false, true) == -1;
    }

    /**
     * Compares two ItemStacks, and tells where the difference is. OreDictionary
     * compatible
     * 
     * @param s1
     *            First stack to compare
     ...
     *         4 If one is null <br />
     */
    public static int compareItemStacksWithOreDict(final ItemStack s1, final ItemStack s2, final boolean amountStrict, final boolean notNull) {
        final int compare = UtilAFM.compareItemStacks(s1, s2, amountStrict, notNull);
        switch (compare) {
            case 1:
            case 2:
            case 3:
            ...
            default:
                return compare; // If one null, already equal, or different amounts,
                                // return it
        }
    }

    /**
     * Checks if both ISs are the same item, regardless the stack size. (Both null returns
     * false)
     * 
     * @param i1
     *            First IS to compare
     ...
     */
    public static boolean isSameItemOreDict(final ItemStack i1, final ItemStack i2) {
        return UtilAFM.compareItemStacksWithOreDict(i1, i2, false, true) < 0;
    }

    private static boolean compareOreDict(final ItemStack s1, final ItemStack s2) {
        final String n1 = UtilAFM.getOreDictOreName(s1);
        final String n2 = UtilAFM.getOreDictOreName(s2);
        return !(n1 == null || n2 == null) && n1.equals(n2);
    }

    public static String getOreDictOreName(final ItemStack is) {
        final String ret = UtilAFM.oreDictMap.get(is);
