...
public class TEFabricator extends TEAFM {
    ...
    public ItemStack getStackInSlotOnClosing(final int slotIndex) {
        final ItemStack ret = this.getStackInSlot(slotIndex);
    ...
    public void updateEntity() {
        ...
        if (this.updateCount % 1 == 0 && this.containerFabricator != null) { // Every 1/20
                                                                             // sec, if
                                                                             // tps if
                                                                             // 100%
    ...
    public void readFromNBT(final NBTTagCompound tagCompound) {
        ...
        final NBTTagList tagList = tagCompound.getTagList("Fabricator");
    ...
    public void setContainer(final ContainerFabricator container) {
    ...
    public ItemStack getCraftSlot(final int i) {
    ...
    public void tryCraft() {
        ...
        final ItemStack resultStack = this.getResult().copy();

        for (int i = 0; i < 9; i++) {
            final ItemStack neededStack = this.getCraftSlot(i);
            if (neededStack == null) {
                continue;
            }

            final ItemStack notFound = this.seekItem(neededStack.copy());
    ...
    private void updateArrays() {
        ...
        for (final ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            final IInventory inv = this.getInventoryInSide(dir);
            if (inv == null) {
                continue;
            }
    ...
    private ItemStack[] invToISArray(final IInventory inv, int startIndex, int length, final ForgeDirection dir) {
        ...
        if (inv instanceof TEFabricator) {
            ...
            final int[] slots = ((ISidedInventory) inv).getAccessibleSlotsFromSide(dir.ordinal());

            final ItemStack[] ret = new ItemStack[slots.length];
            int i = 0;
        ...
        if (inv instanceof TileEntityChest) {
            final TileEntityChest chest1 = (TileEntityChest) inv;
        ...
        // If there's a chest adjacent to the chest adjacent to the fabricator, add it to
        // the array
        for (int i = startIndex; i < startIndex + length; i++) {
            final ItemStack itemStack = chest2.getStackInSlot(i);
    ...
    private IInventory getInventoryInSide(final ForgeDirection dir) {
        final World w = this.worldObj;
    ...
    private ItemStack seekItem(ItemStack needed) {
        ...
        for (final ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            final IInventory inv = this.neighbors[dir.ordinal()];
            if (inv == null) {
                continue;
            }
     ...
     */
    private ItemStack seekAndRemoveItemFromArray(final ItemStack stack, final ItemStack[] inv) {
        for (int i = 0; i < inv.length; i++) {
            final ItemStack curr = inv[i];
            if (curr == null) {
                continue;
            }
    ...
    private boolean addResult(final ItemStack stack) {
        int firstEmpty = -1;
        try {
            for (int i = 0; i < 9; i++) {
                final ItemStack storageStack = this.tempStorage[i];
                if (storageStack == null && firstEmpty == -1) {
                    ...
                    final ItemStack newStack = storageStack.copy();
    ...
    private void updateInventories() {
        ...
        for (final ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            ...
            final IInventory inv = this.neighbors[dir.ordinal()];
            final ItemStack[] tempInv = this.neighArrays[dir.ordinal()];
            ...
            if (inv instanceof TEFabricator) {
                startIndex = 10; // If it's a fabricator, skip the crafting and result
                                 // slots
            }
